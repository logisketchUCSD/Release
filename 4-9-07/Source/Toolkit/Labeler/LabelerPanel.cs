...
namespace Labeler
{
	...
	public class LabelerPanel : System.Windows.Forms.Panel
	{
		...
		private void InitializeLabelMenu()
		{
			...
			this.labelButton.BackColor = Color.Beige;
			this.labelButton.Size = new Size(30, 20);
		...
		public void UpdateFragmentCorners(Hashtable fragStrokeToCorners)
		{
			...
			foreach (DictionaryEntry entry in fragStrokeToCorners)
			{
				...
				if (this.strokeToCorners[stroke] == null && corners.Count > 0)
				{
					notEqualOrNull = true;
				}
				else if (this.strokeToCorners[stroke] != null && (int[])corners.ToArray(typeof(int)) != 
					(int[])(this.strokeToCorners[stroke] as ArrayList).ToArray(typeof(int)))
				{
					notEqualOrNull = true;
				}
				...
				if (notEqualOrNull)
				{
					if (corners.Count == 0 && (this.strokeToCorners[stroke] as ArrayList).Count > 0)
		...
		private void InkResize()
		{
			...
			if (strokeWidth != 0 && strokeHeight != 0 && inkWidth != 0 && inkHeight != 0)
			{
				...
				if (/*this.panelWidthBox.Checked ==*/ true)
				{
					...
					sketchInk.Renderer.Move(toMoveX / 2, toMoveY / 2);
					...
					sketchInk.Width -= toMove.X;
					sketchInk.Height -= toMove.Y;

					overlayInk.Renderer.Move(-toMoveX / 2, -toMoveY / 2);
		...
		private void DisplayToolTip(System.Drawing.Point coordinates)
		{
			...
			if (this.sketchInk == null) // || true) ~Re-enabled for demos
				return;
