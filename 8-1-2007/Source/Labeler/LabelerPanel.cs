...
namespace Labeler
{
	...
	public class LabelerPanel : System.Windows.Forms.Panel
	{
        ...
        protected mInkPicture sketchInk;

        protected Microsoft.Ink.InkOverlay overlayInk;

        protected DomainInfo domainInfo;

        protected List<System.Drawing.Point> lassoPoints;
			...
			base()
		{
			...
			this.sketchInk = new mInkPicture();
			...
			this.mIdToSubstroke = new Dictionary<int,Substroke>();
		...
		private void InitializePanel(Sketch.Sketch sketch)
		{
			...
			this.lassoPoints = new List<System.Drawing.Point>();
			this.clicked = false;
		...
		{
			if (this.sketch != null)
				UpdateColors(new List<Substroke>(this.sketch.Substrokes));
		}
			...
			if (this.domainInfo != null && this.sketch != null)
			{
				foreach (Sketch.Substroke substroke in substrokes)
				{
                    ...
                    if(this.substrokeIdToMStroke.ContainsKey(substroke.XmlAttrs.Id.Value))
					    this.substrokeIdToMStroke[substroke.XmlAttrs.Id.Value].DrawingAttributes.Color = bestColor;
				}
			}
		}
			...
			foreach (KeyValuePair<Sketch.Stroke, List<int>> entry in fragStrokeToCorners)
			{
				...
				if (notEqualOrNull)
				{
					...
					foreach (Sketch.Substroke substroke in stroke.Substrokes)
					{
						...
						this.sketchInk.Ink.DeleteStroke(this.substrokeIdToMStroke[substroke.XmlAttrs.Id.Value]);
		...
		private void DisplayToolTip(System.Drawing.Point coordinates)
		{
            ...
            try
            {
                ...
                closestSubstroke = (Substroke)this.mIdToSubstroke[closestMSubstroke.Id];
            }
            catch
            {
		...
		public Dictionary<int, Substroke> MIdToSubstroke
		{
			get
			{
